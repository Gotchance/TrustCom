...
\begin{enumerate}
  ...
  \item We published datasets used for performance evaluation in this paper on the web\footnote{http://csdataset.nict.go.jp/darknet}: data matrices $\bm{D}_t$ introduced in Section~\ref{Data} and the alert results in Section~\ref{Operation}.
\end{enumerate}
...
To measure the dependency structure between random variables, one may obtain correlation coefficients, but it may include a spurious correlation between random variables.
...
In order to facilitate analysis, in this evaluation, all alerts for ports that were obtained as attacks even once were all counted as cyberattacks.
Please note that some of the alerts classified as cyberattacks included survey scans.
...
There are two types of malware used for cyberattacks: (1) malware that forms a botnet and receives a command from a C2 server then executes a network scan, and (2) malware such as worms, computer viruses, etc. that spreads network scanning on a wide area to self-propagate.
...
Finally, the HNS scanned TCP ports 23, 80, 8080, 2480, 5984, and random port~\cite{Netlab_HNS}.
...
Datasets which we used in this paper are available online at \url{http://csdataset.nict.go.jp/darknet}
