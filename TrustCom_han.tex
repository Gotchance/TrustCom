...
Accordingly, studies on malware activity detection methods that can cope with subspecies and unknown attacks promptly and precisely are important.
Our earlier method monitors network traffic arriving at a darknet, estimates cooperativeness of the source host pairs, and detects outliers through anomalous cooperative traffic.
One of the causes of anomalous source host-to-host cooperativeness on darknet traffic is an Internet-wide scanning from malware.
However, although the method is effective for malware activity detection, it can not analyze in real-time.
In this study, we propose an online processing algorithm, making it possible to detect malware activities in real-time.
...
As a result of evaluating malware activity detection accuracy through a limited ground truth, we obtained results of 91.2\% accuracy, 100\% precision, 91.2\% recall, and 95.4\% F-measure.
\end{abstract}
...
Malware have continued to grow and become diverse recently, and the number of variants and unknown attacks is increasing.
In this paper, a malware activity such as network scanning is regarded as a cyber attack.
...
As a conventional method for dealing with actual cyber attacks quickly and precisely by network-based access control and analysis, there are signature based and whitelist/blacklist based access control methods like a intrusion detection system and a firewall.
In general, however, these conventional methods cannot be expected to cope with subspecies and unknown attacks.
...
For the above reasons, studies on malware activity detection methods that can quickly and precisely deal with subspecies and unknown attacks and without human error are important in recent years.
...
A lot of attacks from malware like indiscriminately scattered network scans reach the darknet.
Therefore, by analyzing the darknet, it is easy to grasp what kind of network service was being targeted and the trend of global cyber attacks.
However, since the darknet sensor we operate does not respond to the reached packets, it is difficult for a human to manually distinguish whether those packets are intended for an attack.
Accordingly, in this study, we propose a mechanism to detect indiscriminate attacks reaching darknets like network scans from malware.
The mechanism detected cyber attacks quickly and precisely, including unknown attacks by using an unsupervised machine learning method, so that there is no human error.




Since infected hosts (devices) forming a botnet behave synchronously when they received commands from a command-and-control server~\cite{Akiyama}, our earlier method~\cite{Han} focused on cooperativeness between the source hosts.
Since this method used the library of R language ``glasso''~\cite{Friedman_glasso}, which is an implementation of an algorithm called ``Graphical Lasso'', this method is called ``{\it GLASSO} engine'' hereafter for convenience.
The {\it GLASSO} engine can be seen that a strongly cooperated network campaign is performed from many source hosts in a certain time period judged as an outlier.
One of the causes of anomalous source host-to-host cooperativeness on darknet traffic is a cyber attack using Internet-wide scanning from malware, and the {\it GLASSO} engine can detect such cyber attacks.
This cooperativeness can be estimated by the sparse structure learning algorithm ``Graphical Lasso''~\cite{Friedman}.
In addition, the graphical lasso can be expected to scrape off weak cooperativeness of host pair that depend on coincidence.
Therefore, by usage of this engine we can be released by worrying the communication due to misconfigurations arriving at the darknet and the weak cooperativeness of the host pair, and can concentrate on more essential cooperativeness.


However, since the {\it GLASSO} engine in the earlier work~\cite{Han} was in a batch mode requiring 3 days' darknet traffic data to process; when considering the processing time, the result was delayed 3 days at most until output.
In other words, it could not be said that the batch-mode {\it GLASSO} engine could analyze darknet traffic promptly.
Therefore, in this paper, we propose an online processing algorithm of {\it GLASSO} engine, which makes outlier detection sequentially and in real-time.
Then, we actually operated the {\it GLASSO} engine in real-time using live darknet traffic (raw packets).
Then, since it is important to evaluate an accuracy with any detection method, we also want to evaluate the accuracy of detection results of {\it GLASSO} engine.
In general, however, it is difficult to distinguish whether a packet arriving at the darknet is intended for a cyber attack.
Therefore, we employ one effective method for evaluation.
We made an answer table in the range we knew and evaluated detection results using this limited ground truth and derived the detection accuracy of malware activities of {\it GLASSO} engine.
To sum up, the proposed {\it GLASSO} engine was possible to quickly and precisely grasp indiscriminate cyber attacks like network scans in real-time, automatically, and we thought it would lead to ease a burden of network security operation and security incident response.
...
\begin{enumerate}
  ...
  This was the first and practical evaluation of the detection accuracy of malware activities on darknet, and we explained the details of detected malware activities.
...
The maximum-likelihood estimation of precision matrix $\Sigma^{-1}$ is the inverse matrix of the sample covariance matrix $S$.
...
Here, $r$ is a hyper parameter to decide how much dependency is regarded as noise-derived, and it is possible to adjust the sparsity of the precision matrix to be estimated.
...
The cooperativeness of host pairs means that there is no cooperativeness with that host pair when the time pattern of the number of packets received from each source host is conditionally independent between two hosts.




\subsection{Algorithm of Batch-mode {\it GLASSO} Engine}
We prepare darknet traffic observed over a long period of time (e.g. 3 days traffic) and divided traffic every $T$ seconds to create multiple time slots.
...
It is a hyper parameter to decide how much dependence is considered to be derived from noise, and adjusts a sparsity of an estimated precision matrix $\Sigma^{-1}$.
...
Next, it could be assumed that 1,539 remaining alerts (47 remaing TCP ports) were network scans since many source hosts sent packets to a specific TCP port of many destination IP address cooperatively.
...
\begin{table}[tb]
  \begin{center}
  \caption{Accuracy of Malware Activity Detection of the {\it GLASSO} Engine}
  \label{tab:accuracy}
\begin{tabular}{c|c|c|c}
\hline
...
In this section, we introduced the details of detected malware activities according to three attack types: IoT malware, router vulnerabilities, and other vulnerabilities.
...
\section{Conclusion}
We have proposed a malware activity detection engine which enables real-time processing and respond more quickly that could not be done with the {\it GLASSO} engine in the earlier work.
...
We made limited ground truth, evaluated malware activity detection accuracy of the {\it GLASSO} engine, and introduced the details of that malware activities.
In addition, it was able to detect what could not be detected by the conventional method.
The {\it GLASSO} engine missed several malware activities, but we considered about counter-measures.
