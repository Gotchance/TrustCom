...
Our earlier method monitors network traffic arriving at a darknet, estimates cooperativeness of the source host pairs, and detects outliers through anomaly cooperative traffic.
One of the causes of anomaly source host-to-host cooperativeness on darknet traffic is an Internet-wide scanning from malware.
...
As a result of evaluating malware activity detection accuracy through a limited ground truth, we obtained results of 91.2\% accuracy, 100\% precision, 91.2\% recall, 95.4\% F-measure.
\end{abstract}
...
In general, however, these conventional methods cannot expect to cope with subspecies and unknown attacks.
...
For the above reasons, studies on a malware activity detection method that can quickly and precisely deal with subspecies and unknown attacks and without human error are important in recent years.
...
Therefore, by analyzing the darknet, it is easy to grasp what kind of network service was being targeted and the trend of global cyber attack.
However, since the darknet sensor we operate do not return a response, it is difficult for a human to manually distinguish whether the received packet is intended for an attack.
Accordingly, in this study, we work on a mechanism to mechanically detect indiscriminate attacks reaching darknets like network scans from malware.
The mechanism detected cyber attacks quickly and precisely including subspecies and unknown attacks by using an unsupervised machine learning method, so that there is no human error.
...
The {\it GLASSO} engine can be seen that a strongly cooperated campaign is performed from many source hosts in a certain time period judged as an outlier.
One of the causes of anomaly source host-to-host cooperativeness on darknet traffic is a cyber attack using Internet-wide scanning, and the {\it GLASSO} engine can detect such cyber attacks.
...
Therefore, this engine would not need to consider the communication due to misconfigurations arriving at the darknet and the weak cooperativeness of the host pair, and it can be expected to estimate more essential cooperativeness.
...
Then, we actually operated the {\it GLASSO} engine in real-time using live darknet traffic (raw packets) and evaluated the detection accuracy of malware activity.
In general, it was difficult to distinguish whether a packet arriving at the darknet was intended for a cyber attack.
Therefore, it was difficult to give out the ground truth of cyber attacks from malware in the darknet, but for the evaluation, we made an answer table in the range we understood and evaluated detection result using limited ground truth.
To sum up, the proposed {\it GLASSO} engine was possible to quickly and precisely grasp indiscriminate cyber attacks like network scans in real-time, automatically, and we thought it would lead to ease a burden of network security operation.

\noindent
\textbf{Contribution.}\space\space
This paper offers the following contributions:
\begin{enumerate}
	\item We proposed an online processing algorithm of {\it GLASSO} engine and made outlier detection sequentially and in real-time.(discussed in Section V)
	\item We tuned important parameters in the {\it GLASSO} engine. (discussed in Section V\hspace{-.1em}I)
	\item We operated the {\it GLASSO} engine in real-time using live darknet traffic (raw packets) without delay, and analyzed the detection result by destination TCP ports (by services). (discussed in Section V\hspace{-.1em}I\hspace{-.1em}I-B, C)
  \item We evaluated the detection accuracy of malware activity, and explained the details of detected malware activities. (discussed in Section V\hspace{-.1em}I\hspace{-.1em}I-D, E)
\end{enumerate}
...
We consider a method of processing alerts (outliers) sequentially by processing every time slot, instead of processing multiple time slots at once.
...
Then, $d_t^{(r)}$ is added to a sequence of the past graph density value $\bm{d}^{(r)}$, and if the length of $\bm{d}^{(r)}$ is positive integer $K$, alert judgment (outlier detection) is performed.
...
Then we evaluated a malware activity detection accuracy and described the details of detected malware activities.
...
As shown in Table~\ref{tab:accuracy}, the accuracy of malware activity detection in October 2018 of the {\it GLASSO} engine was 91.2\% accuracy, 100\% precision, 91.2\% recall, 95.4\% F-measure.
